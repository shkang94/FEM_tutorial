%--------------------------------------------------------------------------
%         FEM 2D 4-Node Quadrilateral (Q4) Element Practice Script
%--------------------------------------------------------------------------
% PURPOSE:
%   To practice the 2D 4-Node Quadrilateral (Q4) finite element.
%
% DESCRIPTION:
%   This script reads the FEM input data from a specified .txt file. It 
%   then performs finite element analysis based on the 2D Q4 element 
...
%   nnpe      : Number of nodes per element (4 for Q4)
%   ndpe      : Number of DOFs per element (8 for Q4)
%   nnpl      : Number of nodes per line (for traction)
%   ndim      : Number of spatial dimensions (2 for Q4)
...
inputname = 'CSAL11_Q4_input.txt'; % Input file name
...
for i = 1:length(FORCE_BODY)
    ...
    if ~found
         error('No matching element found for FORCE_BODY on nodes %d-%d-%d-%d', ...
...
for i = 1:length(FORCE_BODY)
    ...
    fbe = zeros(8,1); % Elemental body force vector
    for igp = 1:ngp
        [N] = Nmat_Q4(xi(igp),eta(igp)); % Shape function matrix
        % Shape function derivatives with respect to the natural 
        % coordinates (2 x 4)
        [dNdxi] = SFD_Q4(xi(igp), eta(igp)); 
        % J   : Jacobian matrix (2 x 2)
        % detJ: Jacobian determinant
        [J, detJ] = Jmat(coorde, dNdxi);
        fbe = fbe + N'*b*t*detJ*wi(igp);
    end
...
for iel = 1:nel % Loop for each element
    ...
    for igp = 1:ngp % Loop for each Gaussian quadrature point
        ...
        % detJ: Jacobian determinant
        [J, detJ] = Jmat(coorde, dNdxi);
        % Strain-displacement matrix of 4-node quadrilateral (3 x 8)
        [B] = Bmat_Q4(dNdxi, J);
...
for iel = 1:nel % Loop for each element
    ...
    for igp = 1:ngp % Loop for each Gaussian quadrature point
        ...
        [EXPL] = EXPLmat_Q4_2D4(xi(igp), eta(igp)); % Extrapolation matrix
...
function [N] = Nmat_Q4(xi, eta)
% xi, eta : Natural coordinates
% N       : Shape function matrix (2 x 8)

N = zeros(2,8);

N(1,1) = 0.25*(1-xi)*(1-eta); % N1
N(1,3) = 0.25*(1+xi)*(1-eta); % N2
N(1,5) = 0.25*(1+xi)*(1+eta); % N3
N(1,7) = 0.25*(1-xi)*(1+eta); % N4

N(2,2:2:8) = N(1,1:2:7); % N(2,2:2:8) <- N1,...,N4
...
dNdxi(1,1) = -0.25*(1-eta); % dN1/dxi
dNdxi(1,2) =  0.25*(1-eta); % dN2/dxi
dNdxi(1,3) =  0.25*(1+eta); % dN3/dxi
dNdxi(1,4) = -0.25*(1+eta); % dN4/dxi

dNdxi(2,1) = -0.25*(1-xi);  % dN1/deta
dNdxi(2,2) = -0.25*(1+xi);  % dN2/deta
dNdxi(2,3) =  0.25*(1+xi);  % dN3/deta
dNdxi(2,4) =  0.25*(1-xi);  % dN4/deta

end

function [B] = Bmat_Q4(dNdxi, J)
% dNdxi : Shape function derivatives with respect to the natural 
%        coordinates (2 x 4)
% J     : Jacobian matrix
% B     : Strain-displacement matrix of 4-node quadrilateral (3 x 8)

dNdx = J\dNdxi; % Shape function derivatives with respect to the
                % global coordinates (2 x 4)

B = zeros(3,8);
B(1,1:2:7) = dNdx(1,:); % B(1,1:2:7) <- dN1/dx,...,dN4/dx
B(2,2:2:8) = dNdx(2,:); % B(2,2:2:8) <- dN1/dy,...,dN4/dy
B(3,1:2:7) = dNdx(2,:); % B(3,1:2:7) <- dN1/dy,...,dN4/dy
B(3,2:2:8) = dNdx(1,:); % B(3,2:2:8) <- dN1/dx,...,dN4/dx
...
function [EXPL] = EXPLmat_Q4_2D4(xi, eta)
% xi, eta : Natural coordinates
% EXPL    : Extrapolation matrix for Q4 element with 2 x 2 integration
