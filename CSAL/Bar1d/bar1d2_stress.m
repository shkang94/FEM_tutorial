%--------------------------------------------------------------------------
%    FEM 1D 2-Node Bar Element Practice Script (with Stress Computation)
%--------------------------------------------------------------------------
% PURPOSE:
%   To practice the 1D 2-node bar finite element with stress computation.
%
% DESCRIPTION:
...
for iel = 1:nel % Loop for each element
    ...
    ue = u(cone); % Elemental displacement
    epsxe = (ue(2)-ue(1))/L; % Elemental axial strain
    sigxe = E*epsxe; % Elemental axial stress
