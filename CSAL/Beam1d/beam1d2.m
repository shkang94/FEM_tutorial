...
%   M         : Element-wise moment for diagram [N*m]
%   V         : Element-wise shear force for diagram [N]
...
inputname = 'beam1d2_input.txt'; % Input file name
uscale = 3.0; % Displacement scaling parameter (user-defined)
...
while 1
    ...
    switch current_section
        ...
        case 'PROP' % Scan material and physical properties
            ...
            PROP(end).I = prop_scan(2); % 2nd moment of area [m^4]
...
for i = 1:length(FORCE_NODE)
    ...
    f(2*NODEid-1) = f(2*NODEid-1) + val(1); % Add nodal force (fy)
    f(2*NODEid)   = f(2*NODEid)   + val(2); % Add nodal moment (mz)
end
...
for i = 1:length(FORCE_LINE)
    ...
    f(2*cone(1)-1) = f(2*cone(1)-1) + val*L/2;
    f(2*cone(2)-1) = f(2*cone(2)-1) + val*L/2;
    % Add normal line force (mz)
    f(2*cone(1)) = f(2*cone(1)) + val*L^2/12;
    f(2*cone(2)) = f(2*cone(2)) - val*L^2/12;
end
...
fy     = f(1:2:ndof-1); % Nodal force (y)
mz     = f(2:2:ndof);   % Nodal moment (z)
uy     = u(1:2:ndof-1); % Nodal displacement (y)
thetaz = u(2:2:ndof);   % Nodal rotation (z)

% 2-6. Obtain elemental moment and shear force for diagram
M = zeros(nel,2); % Element-wise moment for diagram [N*m]
V = zeros(nel,2); % Element-wise shear force for diagram [N]
for iel = 1:nel % Loop for each element
    cone = ELEMENT(iel).con; % Connectivity of current element
    coorde = coord(cone,:); % Coordinate of current element
    PROPid = ELEMENT(iel).PROPid; % PROP ID of current element
...
fprintf('- Force\n')
fprintf('   Node ID            Fy            Mz\n')
for ind = 1:nnd
    fprintf('%10i%14.3e%14.3e\n',ind,fy(ind),mz(ind))
end
fprintf('- Displacement\n')
fprintf('   Node ID            Uy        Thetaz\n')
for ind = 1:nnd
    fprintf('%10i%14.3e%14.3e\n',ind,uy(ind),thetaz(ind))
end
fprintf('- Elemental Moment for Diagram\n')
fprintf('  Elem. ID            M1            M2\n')
for iel = 1:nel
    fprintf('%10i%14.3e%14.3e\n',iel,M(iel,1),M(iel,2))
end
fprintf('- Elemental Shear Force Diagram\n')
fprintf('  Elem. ID            V1            V2\n')
for iel = 1:nel
    fprintf('%10i%14.3e%14.3e\n',iel,V(iel,1),V(iel,2))
end
...
% 3-3. Display the Z-rotation on the deformed configuration
figure(3)
patch('Faces', con, 'Vertices', coord_def, 'LineWidth', 3, ...
...
% 3-4. Display the moment diagram
figure(4)
hold on
for iel = 1:nel
    cone = ELEMENT(iel).con;
    plot(coord(cone),M(iel,:),'k-','LineWidth',2);
end
grid on
...
% 3-5. Display the shear force diagram
figure(5)
hold on
for iel = 1:nel
    cone = ELEMENT(iel).con;
    plot(coord(cone),V(iel,:),'k-','LineWidth',2);
end
grid on
