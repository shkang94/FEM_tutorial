%--------------------------------------------------------------------------
%         FEM 2D 6-Node Triangular (TRI6) Element Practice Script
%--------------------------------------------------------------------------
% PURPOSE:
%   To practice the 2D 6-Node Triangular (TRI6) finite element.
%
% DESCRIPTION:
%   This script reads the FEM input data from a specified .txt file. It 
%   then performs finite element analysis based on the 2D TRI6 element 
...
%   nnpe      : Number of nodes per element (6 for TRI6)
%   ndpe      : Number of DOFs per element (12 for TRI6)
%   nnpl      : Number of nodes per line (for traction, 3 for TRI6)
%   ndim      : Number of spatial dimensions (2 for TRI6)
...
for i = 1:length(FORCE_BODY)
    ...
    if ~found
         error('No matching element found for FORCE_BODY on nodes %d-%d-%d-%d-%d-%d', ...
             cone(1), cone(2), cone(3), cone(4), cone(5), cone(6));
    end
end
...
for i = 1:length(FORCE_BODY)
    ...
    for igp = 1:ngp
        [N] = Nmat_TRI6(xi(igp),eta(igp)); % Shape function matrix
        % Shape function derivatives with respect to the natural 
        % coordinates (2 x 6)
        [dNdxi] = SFD_TRI6(xi(igp), eta(igp)); 
...
for iel = 1:nel % Loop for each element
    ...
    for igp = 1:ngp % Loop for each Gaussian quadrature point
        ...
        % detJ: Jacobian determinant
        [J, detJ] = Jmat(coorde, dNdxi);
        % Strain-displacement matrix of 6-node triangle (3 x 12)
        [B] = Bmat_TRI6(dNdxi, J);
...
for iel = 1:nel % Loop for each element
    ...
    for igp = 1:ngp % Loop for each Gaussian quadrature point
        ...
        [EXPL] = EXPLmat_TRI6_2D3(xi(igp), eta(igp)); % Extrapolation matrix
...
for iel = 1:nel
    ...
    con(iel,[1 3 5 4 6 2]) = ELEMENT(iel).con;
end
...
function [ngp, xi, eta, wi] = GP_2DT3
% ngp     : Number of Gaussian quadrature points
% xi, eta : Sets of natural coordinates for Gaussian quadrature (ngp x 1)
% wi      : Sets of weights for Gaussian quadrature (ngp x 1)
...
function [N] = Nmat_TRI6(xi, eta)
% xi, eta : Natural coordinates
% N       : Shape function matrix (2 x 12)
...
N( 1, 1) = l3*(2*l3-1);   % N1
N( 1, 3) = xi*(2*xi-1);   % N2
N( 1, 5) = eta*(2*eta-1); % N3
N( 1, 7) = 4*xi*eta;      % N4
N( 1, 9) = 4*eta*l3;      % N5
N( 1,11) = 4*l3*xi;       % N6

N(2,2:2:12) = N(1,1:2:11); % N(2,2:2:12) <- N1,...,N6
...
function [dNdxi] = SFD_TRI6(xi, eta)
% xi, eta : Natural coordinates
% dNdxi   : Shape function derivatives with respect to the natural 
%           coordinates (2 x 6)
...
dNdxi(1,1) = -4*l3 +1;    % dN1/dxi
dNdxi(1,2) =  4*xi -1;    % dN2/dxi
dNdxi(1,3) =  0;          % dN3/dxi

dNdxi(1,4) =  4*eta;      % dN4/dxi
dNdxi(1,5) = -4*eta;      % dN5/dxi
dNdxi(1,6) =  4*(l3-xi);  % dN6/dxi

dNdxi(2,1) = -4*l3 +1;    % dN1/deta
dNdxi(2,2) =  0;          % dN2/deta
dNdxi(2,3) =  4*eta-1;    % dN3/deta

dNdxi(2,4) =  4*xi;       % dN4/deta
dNdxi(2,5) =  4*(l3-eta); % dN5/deta
dNdxi(2,6) = -4*xi;       % dN6/deta

end

function [B] = Bmat_TRI6(dNdxi, J)
% dNdxi : Shape function derivatives with respect to the natural 
%        coordinates (2 x 6)
% J     : Jacobian matrix
% B     : Strain-displacement matrix of 8-node quadrilateral (3 x 12)

dNdx = J\dNdxi; % Shape function derivatives with respect to the
                % global coordinates (2 x 4)

B = zeros(3,12);
B(1,1:2:11) = dNdx(1,:); % B(1,1:2:11) <- dN1/dx,...,dN6/dx
B(2,2:2:12) = dNdx(2,:); % B(2,2:2:12) <- dN1/dy,...,dN6/dy
B(3,1:2:11) = dNdx(2,:); % B(3,1:2:11) <- dN1/dy,...,dN6/dy
B(3,2:2:12) = dNdx(1,:); % B(3,2:2:12) <- dN1/dx,...,dN6/dx
...
function [EXPL] = EXPLmat_TRI6_2D3(xi, eta)
% xi, eta : Natural coordinates
% EXPL    : Extrapolation matrix for TRI6 element with 3-point integration

EXPL = zeros(6,1);

N1 = 1-2*(xi-1/6)-2*(eta-1/6);
N2 = 2*(xi-1/6);
N3 = 2*(eta-1/6);
